...
export function PlayActions({ tracks, contextId, pinItem }: Props) {
  ...
  // collapse Play + Shuffle to icon-only so all four buttons stay on ONE tidy row
  ...
  const shareEntries = [
    {
      ...
      icon: <Link2 size={14} />,
      onSelect: () => shareSpotifyLink(shareKind, pinItem.id),
    },
    {
      label: "Copy universal link",
      icon: <Globe size={14} />,
      onSelect: () => shareUniversalLink(shareKind, pinItem.id),
    },
  ];
    ...
    >
      ...
      <motion.button
        ...
        style={{
          ...
          color: pinned ? "#fff" : "var(--color-text)",
          cursor: "pointer",
        }}
      >
