...
export function QueuePanel() {
  ...
  const { queue, history, removeAt, reorder, clearQueue, clearHistory } =
    useQueueStore();
  ...
  return (
    <motion.div
      // absolute overlay, slides via transform. layout space is reserved by the
      // in-flow spacer in Layout.tsx (instant on open AND close) so the grid
      // reflows once and cards glide both ways (see LyricsPanel for the full note).
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          ...
          <SectionHead
            label="Next up"
            onClear={queue.length > 0 ? clearQueue : undefined}
          />
          <div style={{ padding: "0 4px" }}>
            {queue.length === 0 ? (
              <p
