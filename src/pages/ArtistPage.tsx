...
import { useIsArtistFollowed, useToggleFollow, useSavedTrackIds, useToggleLike } from "../hooks/useLibrary";
...
export default function ArtistPage() {
  ...
  const shareEntries = [
    { label: "Copy Spotify link",   icon: <Link2 size={14} />, onSelect: () => id && shareSpotifyLink("artist", id) },
    { label: "Copy universal link", icon: <Globe size={14} />, onSelect: () => id && shareUniversalLink("artist", id) },
  ];
  ...
  if (error) {
    return (
      <div className="flex flex-col gap-4">
        <p className="text-sm" style={{ color: "var(--color-danger)" }}>{errMsg(error)}</p>
      </div>
    );
  }
  if (!data) return null;
  ...
  function startTop(index: number) {
    const start = playContext(showAllTop ? topTracks : topTracks.slice(0, TOP_TRACKS_COLLAPSED), index, `artist-top-${data!.id}`);
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 24, flexWrap: "wrap", minWidth: 0 }}>
        <div style={{ flexShrink: 0 }}>
          <CoverArt url={data.image_url} alt={data.name} size={200} rounded />
        </div>
        <div className="flex flex-col gap-2 min-w-0" style={{ flex: "1 1 260px" }}>
          <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "var(--color-text-dim)" }}>
            Artist
          </p>
          <h1 className="font-black" style={{ fontSize: "clamp(28px, 5vw, 52px)", lineHeight: 1.05, letterSpacing: "-0.02em" }}>
            {data.name}
          </h1>
          ...
          {data.genres.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {data.genres.slice(0, 5).map((g) => (
                <span
                  key={g}
                  className="rounded-full px-3 py-0.5 text-xs capitalize"
        ...
        <section>
          ...
          <div>
            {shownTop.map((t, i) => (
              <motion.div
                ...
                transition={{ duration: 0.28, delay: Math.min(i, 6) * 0.03, ease: [0.23, 1, 0.32, 1] }}
              >
                <TrackRow
                  track={t}
                  index={i}
          ...
          {topTracks.length > TOP_TRACKS_COLLAPSED && (
            <button
              ...
              style={{
                marginTop: 8, padding: "6px 4px", border: "none", background: "transparent",
                color: "var(--color-text-dim)", fontSize: 13, fontWeight: 600, cursor: "pointer",
              }}
      ...
      {data.singles.length > 0 && (
        <section>
          ...
          <AlbumGrid>
            {data.singles.map((al) => <AlbumCard key={al.id} album={al} />)}
          </AlbumGrid>
        </section>
      )}
      {menuEl}
