    ...
    fn check_catalogue(attributes: &UserAttributes) {
        if let Some(account_type) = attributes.get("type") {
            if account_type != "premium" {
                // PATCHED (spotify-client vendor): upstream calls `exit(1)` here,
                // which nukes the whole host app. The account-type read from
                // product-info can be a false negative (a Premium account
                // surfacing a non-"premium" `type` when authed via a generic
                // Web API client_id). The access point grants audio keys from the
                // real server-side entitlement regardless, so log and continue.
                error!("librespot product-info reports non-premium account type {account_type:?}; continuing anyway (patched).");
            }
        }
    }
